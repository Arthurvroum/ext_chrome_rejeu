...
// Désactivation des écouteurs de messages redondants
// Le gestionnaire principal est dans background.js
/*
...
// Nous conservons uniquement le gestionnaire pour updateReplayProgress
// qui n'est pas redondant avec les autres fichiers
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  console.log('Tab Manager received message:', request.action);
  
  if (request.action === 'updateReplayProgress') {
    updateReplayProgress(request.progressData, sendResponse);
    return true;
  }
  return false;
...
console.log('Tab manager loaded - version 1.5 (redundant event listeners removed)');